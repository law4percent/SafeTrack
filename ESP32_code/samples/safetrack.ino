...
void simSend(String cmd, int wait = 400) {
  sim7600.println(cmd);
  delay(wait);
...
void setupSIM7600() {
  Serial.println("Initializing SIM7600 Network...");
  simSend("AT");
  simSend("AT+CFUN=1");
  simSend("AT+CPIN?");
  simSend("AT+CSQ");
  ...
  simSend("AT+CGDCONT=1,\"IP\",\"" + APN + "\"");
  simSend("AT+CGACT=1,1");

  simSend("AT+NETOPEN", 2000);
}

String getDate() { return "11-21-2025"; }
String getTime() { return "10:44"; }
...
bool checkActionOwnerID() {
  String url =
    "https://" + PROJECT_ID +
    ".firebaseio.com/realDevices/" + DEVICE_ID + "/actionOwnerID.json";
...
void uploadDeviceLogs(float lat, float lng, float alt, float speed) {
  ...
  String url =
    "https://" + PROJECT_ID +
    ".firebaseio.com/deviceLogs/" +
    OWNER_UID + "/" + DEVICE_ID + "/" + date + "/" + time + ".json";
...
void uploadDeviceStatus(float lat, float lng, float alt, int battery, bool sos) {
  String url =
    "https://" + PROJECT_ID +
    ".firebaseio.com/linkedDevices/" +
    OWNER_UID + "/devices/" + DEVICE_ID + "/deviceStatus.json";
